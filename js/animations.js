...
(function() {
  ...
  function initAchievementsParallax() {
    ...
    // Spread/Condense scroll effect for floating items
    let ticking = false;
    
    // Predefined spread directions for each item class
    const spreadDirections = {
      // Revenue section - 4 corners
      'revenue': {
        'item-1': { x: -1, y: -1, centered: false },  // top-left
        'item-2': { x: 1, y: -1, centered: false },   // top-right
        'item-3': { x: -1, y: 1, centered: false },   // bottom-left
        'item-4': { x: 1, y: 1, centered: false }     // bottom-right
      },
      // Mentoring section
      'mentoring': {
        'item-1': { x: 0, y: -1, centered: true },    // top-center
        'item-2': { x: -1, y: -0.5, centered: false }, // upper-left
        'item-3': { x: 1, y: -0.5, centered: false },  // upper-right
        'item-4': { x: -1, y: 1, centered: false },    // bottom-left
        'item-5': { x: 1, y: 1, centered: false }      // bottom-right
      },
      // Awards section
      'awards': {
        'award-item-1': { x: 0, y: -1, centered: true },    // top-center
        'award-item-2': { x: -1, y: -0.5, centered: false }, // upper-left
        'award-item-3': { x: 1, y: -0.5, centered: false },  // upper-right
        'award-item-4': { x: -1, y: 1, centered: false },    // bottom-left
        'award-item-5': { x: 1, y: 1, centered: false }      // bottom-right
      },
      // Publications section
      'publications': {
        'item-1': { x: -1, y: -1, centered: false },  // top-left
        'item-2': { x: 1, y: -1, centered: false },   // top-right
        'item-3': { x: 0, y: 1, centered: true }      // bottom-center
      }
    };
    ...
    function updateParallax() {
      ...
      achievementBlocks.forEach(block => {
        ...
        // Calculate scroll progress through this block
        const blockHeight = rect.height;
        const scrollStart = windowHeight;
        const scrollEnd = -blockHeight;
        const scrollRange = scrollStart - scrollEnd;
        
        const currentPos = rect.top;
        let progress = (scrollStart - currentPos) / scrollRange;
        progress = Math.max(0, Math.min(1, progress));
        
        // Spread amount: increases as you scroll through
        const spreadAmount = progress;
        const spreadDistance = 60; // Max pixels to spread
        
        const achievementType = block.getAttribute('data-achievement');
        const directions = spreadDirections[achievementType];
        
        if (!directions) return;
        
        const floatingItems = block.querySelectorAll('.floating-item');
        const blockTop = rect.top + scrollY;
        
        floatingItems.forEach((item, index) => {
          // Find which item class this has
          let itemClass = null;
          ...
          if (!itemClass || !directions[itemClass]) return;
          
          const dir = directions[itemClass];
          
          // Spread effect
          const spreadX = dir.x * spreadDistance * spreadAmount;
          const spreadY = dir.y * spreadDistance * spreadAmount;
          
          // Parallax effect - each item moves at different speed based on index
          const parallaxSpeed = 0.03 + (index * 0.015); // Varying speeds for depth
          const parallaxOffset = (scrollY - blockTop) * parallaxSpeed;
          
          // Combine spread and parallax
          const totalY = spreadY + parallaxOffset;
          
          if (dir.centered) {
            item.style.transform = `translateX(-50%) translate(${spreadX}px, ${totalY}px) scale(1)`;
          } else {
            item.style.transform = `translate(${spreadX}px, ${totalY}px) scale(1)`;
          }
        });
